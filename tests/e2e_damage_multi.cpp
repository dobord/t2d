...
static coro::task<void> flow(std::shared_ptr<coro::io_scheduler> sched, uint16_t port)
{
    ...
    while (std::chrono::steady_clock::now() < deadline && (!gotMatch || !destroyed)) {
    ...
    assert(gotMatch);
    assert(damageEvents >= 1); // at least one damage event
    assert(destroyed); // lethal sequence should occur
    std::cout << "e2e_damage_multi OK" << std::endl;
    co_return;
}
