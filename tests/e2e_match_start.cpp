...
static coro::task<void> client_flow(std::shared_ptr<coro::io_scheduler> sched, uint16_t port){
    ...
    // Give the listener coroutine a brief moment to bind the port before connecting.
    ...
    std::string payload; auth.SerializeToString(&payload);
    ...
    while(std::chrono::steady_clock::now() < deadline && (!gotMatch || !gotSnapshot)) {
        ...
        if(rs == coro::net::recv_status::would_block) continue;
        if(rs == coro::net::recv_status::closed) break;
        ...
        while(t2d::netutil::try_extract(fps, pl)) {
            t2d::ServerMessage sm; bool parsed = sm.ParseFromArray(pl.data(), (int)pl.size()); assert(parsed);
        if(sm.has_auth_response()) { gotAuth = true; std::cout << "[e2e] got AuthResponse" << std::endl; }
        else if(sm.has_queue_status()) { gotQueue=true; std::cout << "[e2e] got QueueStatus" << std::endl; }
        else if(sm.has_match_start()) { gotMatch=true; std::cout << "[e2e] got MatchStart" << std::endl; }
        else if(sm.has_snapshot()) { gotSnapshot=true; std::cout << "[e2e] got Snapshot tick=" << sm.snapshot().server_tick() << std::endl; }
            else { std::cout << "[e2e] got other server msg" << std::endl; }
        }
    }
    assert(gotAuth);
    assert(gotQueue);
