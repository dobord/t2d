...
coro::task<void> run_matchmaker(std::shared_ptr<coro::io_scheduler> scheduler, MatchConfig cfg) {
	...
	while(true) {
		...
		if(!queued.empty() && queued.size() < cfg.max_players) {
			...
			for(auto &q : queued) if(q->queue_join_time < earliest) earliest = q->queue_join_time;
			auto waited = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - earliest).count();
		...
		if(queued.size() >= cfg.max_players) {
			...
			// Baseline snapshot at server_tick=0 (only to real players)
			{
				 ...
				 for(auto &t : ctx->tanks) {
				 	...
				 	ts->set_x(t.x); ts->set_y(t.y);
				 	ts->set_hull_angle(t.hull_angle); ts->set_turret_angle(t.turret_angle);
				 	ts->set_hp(t.hp); ts->set_ammo(t.ammo);
				 }
				 ctx->last_full_snapshot_tick = 0; // baseline considered full base
				 ctx->last_sent_tanks = ctx->tanks;
				 for(auto &s : group) if(!s->is_bot) mgr.push_message(s, base);
			}
