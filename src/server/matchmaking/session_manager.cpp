...
SessionManager &instance() {
    static SessionManager inst; return inst;
}
...
void SessionManager::pop_from_queue(const std::vector<std::shared_ptr<Session>>& sessions) {
    ...
    m_queue.erase(std::remove_if(m_queue.begin(), m_queue.end(), [&](auto &sp){
        return std::find(sessions.begin(), sessions.end(), sp) != sessions.end();
    }), m_queue.end());
    for(auto &s : sessions) {
        s->in_queue = false;
    }
...
void SessionManager::push_message(const std::shared_ptr<Session>& s, const t2d::ServerMessage &msg) {
    std::scoped_lock lk{m_mutex};
    if(s->is_bot) return; // bots do not receive network messages (prototype)
...
void SessionManager::update_input(const std::shared_ptr<Session>& s, const t2d::InputCommand &cmd) {
    std::scoped_lock lk{m_mutex};
    if(cmd.client_tick() < s->input.last_client_tick) return; // ignore old
...
std::vector<std::shared_ptr<Session>> SessionManager::snapshot_all_sessions() {
    ...
    for(auto &kv : m_by_session) res.push_back(kv.second);
    return res;
}
...
void SessionManager::disconnect_session(const std::shared_ptr<Session>& s) {
    ...
    if(!s->session_id.empty()) m_by_session.erase(s->session_id);
