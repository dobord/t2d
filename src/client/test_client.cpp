...
static coro::task<void> send_frame(coro::net::tcp::client &client, const t2d::ClientMessage &msg) {
    std::string payload;
    if(!msg.SerializeToString(&payload)) co_return;
...
static coro::task<bool> read_frame(coro::net::tcp::client &client, t2d::ServerMessage &out) {
    ...
    if(st == coro::net::recv_status::closed) co_return false;
    if(st == coro::net::recv_status::would_block) co_return false;
    if(st != coro::net::recv_status::ok) co_return false;
    state.buffer.insert(state.buffer.end(), span.begin(), span.end());
    std::string payload;
    if(!t2d::netutil::try_extract(state, payload)) co_return false;
    if(!out.ParseFromArray(payload.data(), (int)payload.size())) co_return false;
    co_return true;
}
...
static coro::task<void> client_flow(std::shared_ptr<coro::io_scheduler> scheduler, uint16_t port) {
    ...
    std::cout << "Connected\n";
    // Auth
    t2d::ClientMessage authMsg; auto *ar = authMsg.mutable_auth_request();
    ar->set_oauth_token("dummy"); ar->set_client_version("dev");
    co_await send_frame(cli, authMsg);
    // Queue join
    t2d::ClientMessage q; q.mutable_queue_join();
    co_await send_frame(cli, q);
    // Wait for MatchStart
    auto start = std::chrono::steady_clock::now();
    while(std::chrono::steady_clock::now() - start < 10s) {
        t2d::ServerMessage sm;
        if(!co_await read_frame(cli, sm)) continue;
        ...
        std::cout << "MatchStart id=" << sm.match_start().match_id() << " seed=" << sm.match_start().seed() << std::endl;
            co_return;
        }
    }
...
int main(int argc, char** argv) {
    uint16_t port = 40000; if(argc>1) port = static_cast<uint16_t>(std::stoi(argv[1]));
