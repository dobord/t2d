...
_should_log() {
	local sev=$1
	local sv=$(_level_value "$sev")
	if [[ $sv -ge $_threshold ]]; then return 0; else return 1; fi
}
log() {
	if _should_log INFO; then echo "[$(_ts)] [$APP_ID] [I] $*"; fi
	return 0
}
log_warn() {
	if _should_log WARN; then echo "[$(_ts)] [$APP_ID] [W] $*"; fi
	return 0
}
log_err() {
	if _should_log ERROR; then echo "[$(_ts)] [$APP_ID] [E] $*" >&2; fi
	return 0
}
log_debug() {
	if _should_log DEBUG; then echo "[$(_ts)] [$APP_ID] [D] $*"; fi
	return 0
}
log_trace() {
	if _should_log TRACE; then echo "[$(_ts)] [$APP_ID] [T] $*"; fi
	return 0
}
...
run_once() {
	...
	wait_port || {
		log "Server failed to open port"
