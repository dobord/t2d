...
build_openssl_for_abi() {
	...
	if [[ ! -f $TARBALL_PATH ]]; then
		...
		for u in "${URLS[@]}"; do
			if curl -fsSL "$u" -o "$TARBALL_PATH.tmp"; then
				mv "$TARBALL_PATH.tmp" "$TARBALL_PATH"
				break
			fi
		done
		[[ -f $TARBALL_PATH ]] || {
			echo "[openssl] download failed" >&2
			return 1
		}
	fi
	...
	[[ -d $TOOLCHAIN ]] || {
		echo "[openssl] toolchain missing: $TOOLCHAIN" >&2
		return 1
	}
	...
	x86_64)
		...
		;;
	*)
		echo "[openssl] unsupported ABI $ABI" >&2
		return 1
		;;
	esac
	...
	cp -r "$SRC_DIR" "$BUILD_DIR/openssl"
	...
	if [[ -f "$INSTALL_DIR/lib/libssl.a" && -f "$INSTALL_DIR/lib/libcrypto.a" ]]; then
		...
		echo "[openssl] missing libs for $ABI" >&2
		return 1
	fi
}
...
maybe_build_openssl() {
	...
	command -v curl >/dev/null 2>&1 || {
		echo "curl required for --with-openssl" >&2
		exit 1
	}
	command -v perl >/dev/null 2>&1 || {
		echo "perl required for --with-openssl" >&2
		exit 1
	}
	local NDK_PATH
	if [[ -n ${ANDROID_NDK_HOME:-} ]]; then NDK_PATH="$ANDROID_NDK_HOME"; elif [[ -n ${ANDROID_NDK_ROOT:-} ]]; then NDK_PATH="$ANDROID_NDK_ROOT"; else
		echo "ANDROID_NDK_HOME/ROOT not set (required for OpenSSL build)" >&2
		exit 1
	fi
...
if [[ -z $SDK_ROOT ]]; then
	...
	if [[ -d /opt/android-sdk ]]; then
		...
		info "Guessed ANDROID_SDK_ROOT=$SDK_ROOT"
...
if [[ -z $SDK_ROOT ]]; then
	err "ANDROID_SDK_ROOT / ANDROID_HOME not set (install SDK or export path)"
	exit 1
fi
